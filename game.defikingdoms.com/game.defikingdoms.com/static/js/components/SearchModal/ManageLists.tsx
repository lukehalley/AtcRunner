import { memo, useCallback, useMemo, useRef, useState, useEffect, ChangeEvent } from 'react'
...
import { acceptListUpdate, removeList, disableList, enableList } from 'features/lists/actions'
import { useIsListActive, useAllLists, useActiveListUrls } from 'features/lists/hooks'
...
const ListRow = memo(function ListRow({ listUrl }: { listUrl: string }) {
  const { byUrl } = useSelector(s => s.lists)
  const dispatch = useDispatch()
  const { current: list, pendingUpdate: pending } = byUrl[listUrl]
      ...
      <Column style={{ flex: '1' }}>
        ...
        <RowFixed mt="4px">
          ...
          <StyledMenu ref={node as any}>
            <ButtonEmpty onClick={toggle} ref={setReferenceElement} padding="0" style={{ border: 0 }}>
              <Settings stroke={isActive ? theme.bg1 : theme.text1} size={12} style={{ stroke: `#ccc` }} />
            </ButtonEmpty>
            {open && (
              <PopoverContainer show={true} ref={setPopperElement as any} style={styles.popper} {...attributes.popper}>
                ...
                <UnpaddedLinkStyledButton onClick={handleRemoveList} disabled={Object.keys(byUrl).length === 1}>
                  Remove list
          ...
          <Card backgroundColor={theme.bg2} padding="12px 20px">
            <RowBetween>
              ...
              {isImported ? (
                ...
                <Button style={{ padding: '6px 8px', fontSize: '14px' }} onClick={handleImport} fullWidth>
                  Import
                </Button>
              )}
            </RowBetween>
