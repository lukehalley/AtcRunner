    ...
    <div style={{ padding: '0 10px' }}>
      <div className={styles.col}>
        ...
        <div className={`${styles.statList}`}>
          {stats.map(stat => {
            return <GrowthStat key={stat.abbr} hero={hero} stat={stat} position={'primary'} labelType="abbr" />
          })}
        </div>
      </div>
      <div className={styles.col}>
        ...
        <div className={`${styles.statList}`}>
          {stats.map(stat => {
            return <GrowthStat key={stat.abbr} hero={hero} stat={stat} position={'secondary'} labelType="abbr" />
          })}
        </div>
      </div>
    </div>
...
export default memo(HeroStatsGrowth)
