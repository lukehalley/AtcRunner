...
import { OriginRealm } from 'features/heroes/types'
import { convertHeroId } from 'features/heroes/utils'
import { useDispatch, useSelector } from 'features/hooks'
import { setSelectedStatBoostHero, setShowSurvivorStatBoostModal } from 'features/journey/state'
...
const HeroCard = ({ isFlipped, hero, isAnimated }: HeroCardProps) => {
  ...
  const { statBoostHeroes } = useSelector(s => s.journey)
  const { customCursor } = useSelector(s => s.preferences)
  const hasStatBoost = statBoostHeroes.filter(boostHero => hero && boostHero.id === hero.id).length > 0

  const redirectToStatBoost = () => {
    dispatch(setSelectedStatBoostHero(hero))
    dispatch(setShowSurvivorStatBoostModal(true))
  }
  ...
  let dataPages = [
    {
      ...
      content: (
        <>
          ...
          {hero && hero.owner.name && <div className={styles.heroOwner}>Owned by: {hero.owner.name}</div>}
          {hero && hero.owner.owner && <div className={styles.heroHash}>{hero.owner.owner}</div>}
        </>
      )
    }
  ]

  let statSliders = null
    ...
    dataPages = [
      {
        ...
        content: (
          <>
            <HeroStatsSkills hero={hero} />
            {hero && hero.owner.name ? <div className={styles.heroOwner}>Owned by: {hero.owner.name}</div> : null}
            {hero && hero.owner.owner ? <div className={styles.heroHash}>{hero.owner.owner}</div> : null}
          </>
        )
      },
      { label: 'growth', icon: growthIcon, content: <HeroStatsGrowth hero={hero} /> }
    ]

    statSliders = (
      <>
        <div
          className={`${styles.sliderToggle} ${styles.sliderLeft} ${customCursor ? styles.sliderToggleCursor : null}`}
        ...
        <div
          className={`${styles.sliderToggle} ${styles.sliderRight} ${customCursor ? styles.sliderToggleCursor : null}`}
        ...
        <div className={styles.statsToggle}>
          {dataPages.map((page, index) => {
            return (
              <div key={page.label} className={`${styles.statsToggleButton}`} onClick={() => setDataPageIndex(index)}>
                <div
                  className={`
                    ${styles.statsToggleButtonImage}
                    ${dataPages[dataPageIndex].label === page.label ? styles.statsToggleButtonImageActive : 'inactive'}
                    ${customCursor ? styles.sliderToggleCursor : null}
                  `}
                  style={{
  ...
  return (
    <>
      {hero && (
        <CardContainer key={Number(hero.id)}>
          <div
            className={`
          ${styles.heroCard}
          ...
          >
            <div className={styles.heroCardFront}>
              {hasStatBoost && (
                <button className={styles.perilousJourneyIcon} onClick={redirectToStatBoost}>
                  <img src={boostIcon} alt="" />
                </button>
              )}
              {!hasStatBoost && hero.pjstatus === 'SURVIVED' && (
                <img className={styles.perilousJourneyIcon} src={survivorIcon} alt="" />
              )}
              <div className={styles.heroID}>
                {hero.originrealm && (
                  <img
                    className={styles.cuteJewel}
                    src={hero.originrealm === OriginRealm.CRYSTALVALE ? cuteCrystal : cuteJewel}
                    alt=""
                  />
                )}{' '}
                #{convertHeroId(hero.id)}
              </div>
              ...
              <div className={styles.heroCardFrame}>
                <div className={`${styles.specials} ${styles.row}`}>
                  ...
                  <div className={styles.icon}>
                    {hero.visualGenes.background == 'arctic' && <img src={arcticIcon} />}
                    {hero.visualGenes.background == 'city' && <img src={cityIcon} />}
                    {hero.visualGenes.background == 'desert' && <img src={desertIcon} />}
                    {hero.visualGenes.background == 'forest' && <img src={forestIcon} />}
                    {hero.visualGenes.background == 'island' && <img src={islandIcon} />}
                    {hero.visualGenes.background == 'mountains' && <img src={mountainIcon} />}
                    {hero.visualGenes.background == 'plains' && <img src={plainsIcon} />}
                    {hero.visualGenes.background == 'swamp' && <img src={swampIcon} />}
                    <span className={styles.tooltip}>{hero.background}</span>
                  </div>
                  <div className={styles.icon}>
                    <img src={hero.gender == 'female' ? femaleIcon : maleIcon} />
                ...
                <div className={styles.heroInfo}>
                  ...
                  <div className={styles.cardRarity}>
                    <div className={styles.icon}>
                      ...
                      {hero.rarity == 'legendary' && <img src={legendaryIcon} />}
                      {hero.rarity == 'mythic' && <img src={mythicIcon} />}
                      <span className={styles.tooltip}>{hero.rarity}</span>
                    </div>
                  </div>
                  <div className={styles.level}>
                    {t('Level {{level}}', { level: hero.level })}
                    <span className={styles.subClass}>Gen {hero.generation}</span>
                  </div>
                </div>
            ...
            <div className={styles.heroCardBack}>
              <div className={styles.heroCardFrame}>
                ...
                <div className={styles.heroStats}>
                  <div className={styles.heroFrame}>{dataPages[dataPageIndex].content}</div>
                </div>
              </div>
              {statSliders}
            </div>
