...
import { getClickCursor } from 'features/preferences/utils'
...
export const CloseIcon = styled(X)<{ onClick?: () => void }>`
  cursor: ${() => (store.getState().preferences.customCursor ? `url(${getClickCursor()}), auto` : 'pointer')};
`

export const CloseLetterIcon = styled(X)<{ onClick: () => void }>`
  cursor: ${() => (store.getState().preferences.customCursor ? `url(${getClickCursor()}), auto` : 'pointer')};
  position: absolute;
  right: 16px;
  top: 16px;
