...
(function (SwapPools) {
    ...
    const makeSwapToken = (args) => new SwapToken({
        decimals: 18,
    ...
    const makeETHSwapToken = (args) => new ETHSwapToken({
        decimals: 18,
    ...
    class ETHSwapToken extends SwapToken {
        nativeTokens;
        depositTokens;
        _swapETHAddress = null;
        constructor(args) {
            let { swapEthAddress, nativeTokens, depositTokens, ...constructorArgs } = args;
    ...
    const makeSingleTokenPool = (t) => ({ poolTokens: [t], swapType: t.swapType });
    const ETH_Pool = makeSingleTokenPool(Tokens.ETH), SYN_Pool = makeSingleTokenPool(Tokens.SYN), FRAX_Pool = makeSingleTokenPool(Tokens.FRAX), HIGH_Pool = makeSingleTokenPool(Tokens.HIGH), DOG_Pool = makeSingleTokenPool(Tokens.DOG), JUMP_Pool = makeSingleTokenPool(Tokens.JUMP), NFD_Pool = makeSingleTokenPool(Tokens.NFD), GOHM_Pool = makeSingleTokenPool(Tokens.GOHM), GMX_Pool = makeSingleTokenPool(Tokens.GMX), SOLAR_Pool = makeSingleTokenPool(Tokens.SOLAR), AVAX_Pool = makeSingleTokenPool(Tokens.AVAX), WAVAX_Pool = makeSingleTokenPool(Tokens.WAVAX), SYN_AVAX_Pool = makeSingleTokenPool(Tokens.SYN_AVAX), MOVR_Pool = makeSingleTokenPool(Tokens.MOVR), WMOVR_Pool = makeSingleTokenPool(Tokens.WMOVR), UST_Pool = makeSingleTokenPool(Tokens.UST), NEWO_Pool = makeSingleTokenPool(Tokens.NEWO), SDT_Pool = makeSingleTokenPool(Tokens.SDT), LUNA_Pool = makeSingleTokenPool(Tokens.LUNA), USDB_Pool = makeSingleTokenPool(Tokens.USDB), DFK_USDC_Pool = makeSingleTokenPool(Tokens.DFK_USDC), XJEWEL_Pool = makeSingleTokenPool(Tokens.XJEWEL), AVAX_JEWEL_Pool = { poolTokens: [Tokens.JEWEL, Tokens.MULTIJEWEL], swapType: SwapType.JEWEL }, DFK_JEWEL_Pool = { poolTokens: [Tokens.GAS_JEWEL, Tokens.JEWEL], swapType: SwapType.JEWEL }, HARMONY_JEWEL_Pool = { poolTokens: [Tokens.JEWEL, Tokens.SYN_JEWEL], swapType: SwapType.JEWEL };
    const makeSwapTypeMap = (base, ...pools) => {
        let m = {
            swappableTokens: {
    ...
    SwapPools.bridgeSwappableMap = {
        [ChainId.ETH]: makeSwapTypeMap({
            usdPool: [
                SwapPools.ETH_POOL_SWAP_TOKEN,
                [...SwapPools.ETH_POOL_SWAP_TOKEN.poolTokens, Tokens.NUSD]
            ]
        }, ETH_Pool, HIGH_Pool, DOG_Pool, FRAX_Pool, NEWO_Pool, SDT_Pool, USDB_Pool),
        ...
        [ChainId.MOONRIVER]: makeSwapTypeMap({}, SOLAR_Pool, FRAX_Pool, MOVR_Pool, USDB_Pool),
        ...
        }, FRAX_Pool, SDT_Pool, SYN_AVAX_Pool, XJEWEL_Pool, HARMONY_JEWEL_Pool)
    };
    function swapGroupsForChain(chainId) {
        return Object.values(SwapPools.bridgeSwappableMap[chainId].swappableSwapGroups).map(lp => lp.swapType);
    }
    SwapPools.swapGroupsForChain = swapGroupsForChain;
    function tokensForChainBySwapGroup(chainId, swapGroup) {
        const m = SwapPools.bridgeSwappableMap[chainId].swappableTokens;
...
export function networkSwapTokensMap(chainIdA, chainIdB) {
    let res = {};
    const swapGrpsA = SwapPools.swapGroupsForChain(chainIdA);
...
export function allNetworksSwapTokensMap() {
    let res = {};
    supportedChainIds().forEach((chainIdA) => {
        const swapGrpsA = SwapPools.swapGroupsForChain(chainIdA);
