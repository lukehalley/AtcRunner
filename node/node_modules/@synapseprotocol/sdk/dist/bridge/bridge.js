...
import { SwapType, rpcProviderForChain, tokenSwitch } from "../internal/index.js";
...
(function (Bridge) {
    ...
    class SynapseBridge {
        ...
        isL2ETHChain;
        zapBridgeAddress;
        bridgeConfigInstance = SynapseEntities.BridgeConfigV3ContractInstance();
        zapBridgeInstance = SynapseEntities.L1BridgeZapContractInstance({
            chainId: ChainId.ETH,
        ...
        constructor(args) {
            ...
            const factoryParams = { chainId: this.chainId, signerOrProvider: this.provider }, contractAddrs = SynapseContracts.contractsForChainId(this.chainId);
        ...
        async calculateBridgeRate(args) {
            ...
            const toChainZap = SynapseEntities.GenericZapBridgeContractInstance(toChainZapParams);
            ...
            const intermediateTokenAddr = bridgeConfigIntermediateToken.address(chainIdTo).toLowerCase(), multiplier = BigNumber.from(10).pow(18 - fromCoinDecimals), amountFromFixedDecimals = amountFrom.mul(multiplier);
            const bridgeFeeRequest = this.bridgeConfigInstance["calculateSwapFee(string,uint256,uint256)"](intermediateTokenAddr, chainIdTo, amountFromFixedDecimals);
            ...
            if (amountFrom.isZero()) {
                ...
                amountToReceive_from_prom = BridgeUtils.calculateSwapL2Zap(this.networkZapBridgeInstance, intermediateToken.address(this.chainId), tokenIndexFrom, 0, amountFrom);
            }
            ...
            if (amountToReceive_from.isZero()) {
                ...
                amountToReceive_to_prom =
                    toChainZap.calculateRemoveLiquidityOneToken(amountToReceive_from, tokenIndexTo);
            }
            else {
                amountToReceive_to_prom = BridgeUtils.calculateSwapL2Zap(toChainZap, intermediateToken.address(chainIdTo), 0, tokenIndexTo, amountToReceive_from);
            }
            let amountToReceive;
            try {
        ...
        buildETHMainnetBridgeTxn(args, tokenArgs) {
            const { addressTo, chainIdTo, amountFrom, amountTo } = args, zapBridge = SynapseEntities.L1BridgeZapContractInstance({
            ...
            if (args.tokenTo.isEqual(Tokens.NUSD)) {
                if (args.tokenFrom.isEqual(Tokens.NUSD)) {
                    const bridgeDepositArgs = BridgeUtils.makeEasyParams(args, this.chainId, args.tokenTo);
                    return zapBridge
                        .populateTransaction
            ...
            if (BridgeUtils.isETHLikeToken(args.tokenTo) || args.tokenTo.isEqual(Tokens.WETH)) {
                ...
                return zapBridge.populateTransaction.depositAndSwap(addressTo, chainIdTo, Tokens.NUSD.address(this.chainId), amountFrom, 0, tokenArgs.tokenIndexTo, minToSwapDestFromOriginMediumSlippage, bridgeTransactionDeadline);
            }
        ...
        buildL2BridgeTxn(args, tokenArgs) {
            ...
            BridgeUtils.DepositIfChainTokens.forEach((args) => {
                ...
                let hasAltChain = typeof altChainId !== 'undefined', tokenHashes = tokens.map((t) => t.id);
